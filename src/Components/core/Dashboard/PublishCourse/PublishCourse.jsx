...
const PublishCourse = () => {
    ...
    const handelPublish = async () => {
        ...
        // Only add to category if the course is being published and has a category
        if (result && getValues("public") && course.category) {
            const category_id = course.category._id || course.category;
            console.log("Adding course to category:", category_id);
            ...
            if (addCourseCategory) {
                ...
                toast.error("Course published but failed to add to category");
                goToMyCourses();
                dispatch(setStep(1));
                dispatch(setEditCourse(null));
                setLoading(false);
